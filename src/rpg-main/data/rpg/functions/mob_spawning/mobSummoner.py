...
def createMob(NameSpaceBuf, SpaceBuf, NameBuf, CmdBuf, FeatureVal, HasFeature): 
    ...
    set_mob = open('./set_mob_one.mcfunction', mode="at", encoding="utf-8")
    if HasFeature == 0:
    ...
    if HasFeature == 1 and FeatureVal == 0:
        process = open("./mobs/%sprocess.mcfunction" %(SpaceBuf), mode="wt", encoding="utf-8")
        processFunction(process, NameSpaceBuf, SpaceBuf, NameBuf)
        process.close()
    
    #File Writing In Mob Summon File
    mob_file = open('./mobs/%s%s.mcfunction' %(SpaceBuf, NameBuf), mode="wt", encoding="utf-8")
