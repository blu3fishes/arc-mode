...
#if owner exits.
execute as @e if score @s arc.UUID0 = .search arc.UUID0 if score @s arc.UUID1 = .search arc.UUID1 if score @s arc.UUID2 = .search arc.UUID2 if score @s arc.UUID3 = .search arc.UUID3 run function arc_mob_base:custom_ranged/owner
execute if score .type arc.BowType matches 1.. run function arc_mob_base:custom_ranged/load/directory